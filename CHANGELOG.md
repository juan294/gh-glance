...
- **Package-boundary verification supports npm 12.** The release test accepts
  both npm 11's array-shaped `npm pack --json` result and npm 12's
  package-name-keyed result, so the existing OIDC publication workflow can
  verify the CLI-only package boundary before publishing.
